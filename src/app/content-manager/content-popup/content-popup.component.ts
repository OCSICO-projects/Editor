...
import { MAT_DIALOG_DATA, MatDialog, MatDialogRef, MatIconRegistry, MatSnackBar, MatSnackBarHorizontalPosition, MatSnackBarVerticalPosition } from '@angular/material';
...
export class ContentPopupComponent implements OnInit, OnDestroy {
    ...
    private updateAfterUpload(): void {
        ...
        this.contentManagerService.fileUpload
            ...
            .subscribe(resource => {
                ...
                if (['image', 'video'].indexOf(resource.subtype) !== -1) {
                    resource.disabled = true;
                    this.waitForPreview(resource);
                }

                this.addUploadedResource(resource);
            });
    }

    private waitForPreview(resource) {
        this.echoService.join(`preview.${resource.file.id}`, 'public');
        this.echoService.listen(`preview.${resource.file.id}`, 'PreviewGenerated').pipe(
            first()
        ).subscribe(
            data => {
                this.echoService.leave(`preview.${resource.file.id}`);
                resource.preview_id = data.file.id;
                resource.image = data.file.url;
                resource.preview = data.file.url;
                if (resource.subtype === 'video') {
                    this.waitForConvertedVideo(resource);
                } else {
                    resource.disabled = false;
                }
                this.updateUploadedResource(resource);
            }, error => console.error(error)
        );
    }

    private waitForConvertedVideo(resource) {
        this.echoService.join(`video.${resource.file.id}`, 'public');
        this.echoService.listen(`video.${resource.file.id}`, 'VideoConverted').pipe(
            first()
        ).subscribe(
            data => {
                this.echoService.leave(`video.${resource.file.id}`);
    ...
    private addUploadedResource(resource) {
        if (resource.preview) {
            resource.image = resource.preview;
        }

        resource.icon = resource.subtype;

        if (parseInt(resource.folder_id, 10) === parseInt(this.pathArr[this.pathArr.length - 1].id, 10)) {
            this.list.push(resource);
        }

        if (!resource.folder_id && this.pathArr[this.pathArr.length - 1].isFirst) {
            this.list.push(resource);
        }
    ...
    private updateUploadedResource(resource) {
        this.list = this.list.filter(item => item.id !== resource.id);
        this.list.push(resource);
        this.contentManagerService.onSetList(this.list);
    }
    ...
    private disableUnprocessedFiles(list) {
        return list.map(item => {
            item.disabled = this.resourceIsUnprocessed(item);
            return item;
        });
    }

    private resourceIsUnprocessed(resource) {
        return (['video', 'image'].indexOf(resource.subtype) !== -1 && !resource.preview_id) ||
            (resource.subtype === 'video' && resource.file.filename.indexOf('encoded.mp4') === -1);
    }
    ...
    private handleCurrentList(): void {
        this.contentManagerService.handleResponseCurrentList()
            ...
            .subscribe(list => {
                this.list = this.disableUnprocessedFiles(list);
            });
    }
            ...
            dialogRef.afterClosed().subscribe(result => {
                ...
                this.contentManagerService.updateNode(node).subscribe((updatedNode: Folder | Resource) => {
                    this.list = this.list.filter(el => {
                        return el.selected ? el.id !== updatedNode.id : true;
                    });
    ...
    public ngOnDestroy(): void {
        this.contentManagerService.selectItem(null);
        this.onDestroy$.next();
